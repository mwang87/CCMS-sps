...
namespace specnets
{
  ...
  // -------------------------------------------------------------------------
  bool matchIon(const string &ionName,
                const vector<ftIonFragment> &ionTypes,
  ...
  {
    AAJumps jumps(1);
    return annotate(ionInputMasses,
                    model,
                    peakTol,
                    jumps,
                    removeDuplicates,
                                      ...
                                      const bool retainOldAnnotations)
  {
    ...
    for (short ionIdx = 0; ionIdx < ionInputMasses.size(); ionIdx++)
    {
      ...
      if (!matchIon(ionInputMasses[ionIdx].m_name, m_ionTypes, ionFragmentIndex))
      {
      ...
      else
      {
        ...
        m_spectrum->findMatches(ionInputMasses[ionIdx].m_mass, -1.0, matches);
                                                           ...
                                                           bool useOriginal)
  {
    ...
    while (ia < massValues.size() && is < m_spectrum->size())
    {
      ...
      } while (diffMass > peakTolerance && ia < massValues.size() && is
          < m_spectrum->size());
                                                           ...
                                                           string & annotation)
  {
    ...
    {
      char modString[32];
      sprintf(modString, "%.3f", (*m_spectrum)[m_spectrum->size() - 1][0]
  ...
  // -------------------------------------------------------------------------
  bool PeptideSpectrumMatch::generateTheoreticalMassesWithMods(const string &peptide,
                                                               const string &ionNamesInclude,
                                                               const MS2ScoringModel &inputIonTypes,
                                                               const float prmOffset,
                                                               const float srmOffset,
                                                               ...
                                                               vector<IonMass> &ionOutputMasses)
  {
    ...
    for (ionIdx = 0; ionIdx < peptideLength - 1; ionIdx++)
    {
      ...
      for (currIonFrag = ionOutputFragments.begin(); currIonFrag
          != ionOutputFragments.end(); currIonFrag++)
      {
        if (currIonFrag->isIF)
        {
          IonMass currIonMass;
          ...
          for (int i = 0; i < modCombos.size(); i++)
          {
            ...
            if (ionIdx + 1 >= modCombos[i].size())
            {
              ...
              if (currIonFrag->isNTerm)
              {
                ...
                for (int position = 0; position < fixedPositions.size(); position++)
                {
          ...
          if (currIonFrag->isNTerm)
          {
            ...
            currMass
                = (prmMasses[ionIdx] + currIonFrag->massOffset + totalMass)
                    / currIonFrag->charge;
#ifdef DEBUG
            std::cout << "mass " << currMass << endl;
            std::cout << "name " << ionIdx << " " << currIonFrag->name << endl;
          ...
          else
          {
            ...
            currMass
                = (srmMasses[ionIdx] + currIonFrag->massOffset + totalMass)
                    / currIonFrag->charge;
#ifdef DEBUG
            std::cout << "mass " << currMass << endl;
            std::cout << "name " << ionIdx << " " << currIonFrag->name << endl;
  ...
  // -------------------------------------------------------------------------
  bool PeptideSpectrumMatch::getModifications(vector<float> &modifications, bool useOriginal) const
  {
    vector<unsigned int> positions;
    vector<unsigned int> lengths;
    return getModificationsAndPositions(modifications, positions, lengths, useOriginal);
  }

  // -------------------------------------------------------------------------
  bool PeptideSpectrumMatch::getModificationsAndPositions(vector<float> & modifications,
                                                          vector<unsigned int> & positions, 
							  ...
							  bool useOriginal) const
  {
    ...
    while (start != string::npos)
    {
      ...
      if (annotation.substr(start, 1).compare("(") == 0)
      {
        ...
        {
          ss.clear();
          WARN_MSG("Unable to convert " << annotation.substr(mod+1, end-mod-1) << " to float!");
          return false;
        }
        else
        {
                                                 ...
                                                 string &outputPeptide)
  {
    ...
    for (unsigned int i = 0; i < positions.size(); i++)
    {
      ...
      {
        ERROR_MSG("Modification position outside peptide! positions " << positions[i] << " length " << unmodifiedPeptide.length()+1);
        return false;
      }
                                               ...
                                               string &specnets)
  {
    ...
    if (test == string::npos || (test != 1 && !(firstChar >= '0' && firstChar
        <= '9')))
    {
      stringstream ss;
      ss << "*." << inspect << ".*";
