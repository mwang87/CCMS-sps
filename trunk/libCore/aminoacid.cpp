...
namespace specnets
{
  ...
  const double AAJumps::massHion = 1.007825035;
  ...
  bool AAJumps::getAllAArefs(vector<pair<char, float> > & returnAAs)
  {
    for (int i = 0; i < refLetters.size(); i++)
    {
      pair<char, float> newPair = make_pair<char, float>(refLetters[i], refMasses[i]);
  ...
  {
    vector<float> masses;
    bool sequenceOK = getPRMMasses(sequence, masses, offset, (vector<string> *)0, addZeroMass);
