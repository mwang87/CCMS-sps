...
namespace specnets
{
  ...
  void Spectrum::roundPeaks(float roundFactor)
  {
    for (unsigned int i = 0; i < peakList.size(); i++)
    {
                                   ...
                                   bool useExistingRawScore)
  {
...
#if 0
    // LARS: THIS IS A (TEMPORARY) FIX FOR 0 VALUES IN ARRAY
    if (DEBUG_SPEC_PROB)
    DEBUG_VAR(m_gfProbs.size());
    int firstZeroIndex = m_gfProbs.size() + 1;
    for (int i = m_gfProbs.size() - 1; i > 0; i--)
    {
      if (m_gfProbs[i] == 0.0)
      {
        firstZeroIndex = i;
    ...
    if (DEBUG_SPEC_PROB)
    DEBUG_VAR(firstZeroIndex);
    for (int i = firstZeroIndex; i < m_gfProbs.size(); i++)
    {
      m_gfProbs[i] = m_gfProbs[firstZeroIndex - 1] / 10.0;
    }
#endif
                                              ...
                                              bool useExistingRawScore)
  {
    ...
    if (useExistingRawScore)
    {
      // We need to scale the max score to 100
      // The maximum possible would be the sum of the peaks
    ...
    for (; itr != itr_end; itr++)
    {
      ...
      psm->m_pValue = -log10(psm->m_pValue);
      if (DEBUG_SPEC_PROB)
        DEBUG_VAR(psm->m_pValue);

      if (!isfinite(psm->m_pValue))
      {
        psm->m_pValue = 0.0;
      }
      if (DEBUG_SPEC_PROB)
  ...
  float Spectrum::scoreMatch(Spectrum &spectrum_comparison,
                             float peakTol,
