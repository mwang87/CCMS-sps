...
namespace specnets
{
    ...
    int SpectralLibrary::search(Spectrum &query_spec, PeptideSpectrumMatchSet &output_psms, float parentmass_tolerance, float ion_tolerance, int top_hits, int analog_search, float score_threshold, int library_search_quality){
        //AAJumps aajumps(1);
        vector<float> masses;
        int charge;
        ...
        for(int library_idx = 0; library_idx < specs.size(); library_idx++){
            ...
            float sim = specs[library_idx].scoreMatch(query_spec, ion_tolerance, shared_peaks);
