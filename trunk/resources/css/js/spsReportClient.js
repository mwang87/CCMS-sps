...
// table control array. This variable is meant to contain all the tables' data
// and later parser by the init() function.
var TC = [];
...
// Working image, displayed when the browser is "working"
...
function buildTablePaginationBar(tab, tabData)
{
  ...
  for(var i = 0 ; i < tabData.length ; i += len ) {
    ...
    if(col == scol) {
      ...
      if(first == second)
        str = '[' + first  + ']';
      stripped = str.replace(/(<([^>]+)>)/ig,"");

    } else {
      stripped = '[Page&nbsp;' + (i / len + 1) + ']';
    }
